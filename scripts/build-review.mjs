...
const headingFrom = (text, index) => { const supplied = item.headings?.[index - 1]; if (supplied) return supplied; const first = text.split(/[.!?。！？]/)[0].trim(); return first.length > 28 ? `${first.slice(0, 28).trim()}…` : first; };\nconst body = paragraphs.map((p, i) => `<h2>${i === 0 ? '오늘 방송 핵심 내용' : esc(headingFrom(p, i))}</h2><p>${esc(p)}</p>`).join('');
