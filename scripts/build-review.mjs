...
const headingFrom = (text, index) => { const supplied = item.headings?.[index - 1]; if (supplied) return supplied; const first = text.split(/[.!?。！？]/)[0].trim().replace(/(했습니다|되었습니다|이어졌습니다|소개됐습니다|확인할 수 있습니다|입니다|합니다)$/,'').trim(); return first.length > 30 ? `${first.slice(0, 30).trim()}…` : first; };\nconst splitReadable = text => { const parts = text.split(/(?<=[.!?。！？])\\s+/).filter(Boolean); const chunks = []; for (let i = 0; i < parts.length; i += 2) chunks.push(`<p>${esc(parts.slice(i, i + 2).join(' '))}</p>`); return chunks.join(''); };\nconst body = paragraphs.map((p, i) => `<h2>${i === 0 ? '오늘 방송 핵심 내용' : esc(headingFrom(p, i))}</h2>${splitReadable(p)}`).join('');
