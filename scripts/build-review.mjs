...
const transcriptParagraphs = String(item.transcript).split(/\n{2,}/).map(x => x.trim()).filter(Boolean);
...
const intro = sourceParagraphs[0];
const sections = config.headings.map((heading, index) => ({ heading, paragraphs: sourceParagraphs.slice(index * 2, index * 2 + 2) })).filter(section => section.paragraphs.length);
