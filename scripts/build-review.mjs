...
const headingFrom = text => { const first = text.split(/[.!?。！？]/)[0].trim(); return first.length > 42 ? `${first.slice(0, 42).trim()}…` : first; };\nconst body = paragraphs.map((p, i) => `<h2>${i === 0 ? '오늘 방송 핵심 내용' : esc(headingFrom(p))}</h2><p>${esc(p)}</p>`).join('');
